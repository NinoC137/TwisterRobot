...
{
    Pid_Init(&Balance_PID, 0.5f, 0.25f,0.0f);
    Balance_PID.IntegralMax = 3.0f;
    Pid_Init(&Speed_PID, 1.2f, 0.0f,0.3f);
    Speed_PID.IntegralMax = 10.0f;

    /***********************     FOC控制部分   ***********************************/

    Pid_Init(&Motor_Left_position, 2.0f, 0.00f, 0.0f);
    Motor_Left_position.IntegralMax = 10.0f;
    Pid_Init(&Motor_Left_speed, 0.08f, 0.8f, 0.0f);
    Motor_Left_speed.IntegralMax = 6.0f;

    Pid_Init(&Motor_Right_position, 2.0f, 0.00f, 0.0f);
    Motor_Right_position.IntegralMax = 3.0f;
    Pid_Init(&Motor_Right_speed, 0.08f, 0.8f, 0.0f);
